...
ggplot(dat_slim_df, aes(t_2m, gust_time)) + geom_point(color = alpha(rangi2, 0.1)) + geom_line(data = dat_smooth, size = 1) + geom_ribbon(data = dat_smooth, aes(ymin = low, ymax = high), fill = alpha("darkgreen", 0.3))
...
plot( gust_time ~ wind_spd , dat_slim_2 , col=col.alpha(rangi2,0.5) )
lines( wind_spd.seq , mu.mean )
shade( mu.PI , wind_spd.seq )
shade( height.PI , wind_spd.seq )
