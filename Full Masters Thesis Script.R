...
new_combined_data <- mutate(combined_data, t_difference = t_10m-t_2m) #calculate temperature difference between 10m above sea level and 2m above sea level
...
updated_Richardson_combined_cc <- mutate(new_combined_data, RichardsonBulk = ((9.81/t_2m)*(t_difference)*(8))/(wind_spd^2))
